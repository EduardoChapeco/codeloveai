...
import {
  ...
  RefreshCw, Unlink, Loader2, ShieldAlert,
  Activity, Key, Globe, Webhook, ExternalLink,
} from "lucide-react";
...
export default function LovableCloudTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="lv-heading-md flex items-center gap-2.5">
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  <th className="text-left px-5 py-2.5 font-bold text-muted-foreground">Usuário</th>
                  <th className="text-left px-4 py-2.5 font-bold text-muted-foreground">Status</th>
                  <th className="text-left px-4 py-2.5 font-bold text-muted-foreground hidden md:table-cell">Última verificação</th>
                  <th className="text-left px-4 py-2.5 font-bold text-muted-foreground hidden lg:table-cell">Criado em</th>
              ...
              <tbody>
                {accounts.map((acc) => {
                  ...
                  return (
                    <tr key={acc.id} className="border-b border-border transition-colors hover:bg-muted/30">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-full flex items-center justify-center text-primary-foreground text-[10px] font-bold shrink-0 bg-primary">
                            {initial}
                          </div>
                          <div>
        ...
        {loadingLogs ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  <th className="text-left px-5 py-2.5 font-bold text-muted-foreground">Endpoint</th>
                  <th className="text-left px-4 py-2.5 font-bold text-muted-foreground">Método</th>
                  <th className="text-left px-4 py-2.5 font-bold text-muted-foreground">Status</th>
                  <th className="text-left px-4 py-2.5 font-bold text-muted-foreground hidden md:table-cell">Duração</th>
                  <th className="text-left px-4 py-2.5 font-bold text-muted-foreground">Hora</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log) => {
                  ...
                  return (
                    <tr key={log.id} className="border-b border-border transition-colors hover:bg-muted/30">
                      <td className="px-5 py-2.5 font-mono text-[11px] text-foreground max-w-[220px] truncate" title={log.endpoint}>
                        {log.endpoint}
                      </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="lv-card space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.label}>
                <p className="lv-caption mb-1">{item.label}</p>
                <div className="flex items-center gap-2">
                  <code className="text-[11px] font-mono text-foreground flex-1 px-3 py-2 rounded-lg truncate bg-muted/60 border border-border">
                    {item.value}
                  </code>
                  {item.value.startsWith("http") && (
                    <a href={item.value} target="_blank" rel="noopener noreferrer" className="lv-btn-icon h-8 w-8 shrink-0">
          ...
          <div>
            <p className="lv-caption mb-2">Prefixos permitidos</p>
            <div className="flex flex-wrap gap-2">
              {PROXY_INFO.allowed_prefixes.map(p => (
                <span key={p} className="lv-badge lv-badge-muted">{p}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="lv-card space-y-4">
          ...
          <p className="lv-caption">
            Todos os requests para <code className="font-mono text-foreground">api.lovable.dev</code> incluem:
          </p>
          <div className="space-y-2">
            {PROXY_INFO.required_headers.map(h => (
              <div key={h} className="px-3 py-2 rounded-lg font-mono text-[11px] text-foreground bg-muted/60 border border-border">
                {h}
              </div>
            ))}
          </div>
